  ...
  (api
   ...
   (context "/api" []
    ...
    (GET "/users/" []
      ...
      (fn [_ respond _]
        (execute! db ["select id, email from users limit 10"]
                  (fn [res err] (respond (ok (:rows res)))))
        #_(go (let [result (<! (execute! db ["select id, email from users limit 10"]))]
              (respond (ok (:rows result)))))))
